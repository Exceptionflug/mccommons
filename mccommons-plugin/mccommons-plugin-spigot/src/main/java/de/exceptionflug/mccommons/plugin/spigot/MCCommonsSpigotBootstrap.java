...
public class MCCommonsSpigotBootstrap {
	...
	public void enable(final JavaPlugin plugin) {
		try {
			...
			try {
				Converters.register(
					ReflectionUtil.getClass("{obc}.entity.CraftPlayer"),
					PlayerWrapper.class,
					new PlayerConverter());
			...
			Converters.register(
				...
				new FlowNbtNmsNbtConverter());
			Converters.register(
				ReflectionUtil.getClass("{nms}.NBTTagCompound"),
				CompoundTag.class,
				new NmsNbtFlowNbtConverter());
			Converters.register(
				ItemStack.class,
				ItemStackWrapper.class,
				src -> new SpigotItemStackWrapper((ItemStack) src));
			Converters.register(
				ItemStackWrapper.class,
				ItemStack.class,
				src -> ((ItemStackWrapper) src).getHandle());
			Converters.register(
				SpigotItemStackWrapper.class,
				ItemStack.class,
				src -> ((ItemStackWrapper) src).getHandle());
			Converters.register(
				ClickType.class,
				de.exceptionflug.mccommons.inventories.api.ClickType.class,
			...
			Converters.register(
				ReflectionUtil.getClass("{obc}.inventory.CraftItemStack"),
				ItemStack.class,
				new CraftItemStackConverter());
