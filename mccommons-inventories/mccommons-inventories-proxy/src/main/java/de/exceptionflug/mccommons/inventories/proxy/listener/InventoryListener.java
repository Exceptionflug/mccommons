...
public class InventoryListener implements Listener {
    ...
    public void onClick(final InventoryClickEvent e) {
        ...
        if (actionHandler == null)
            return;
        try {
            final CallResult callResult = actionHandler.handle(new Click(Converters.convert(e.getClickType(), ClickType.class), wrapper, item.getItemStackWrapper(), e.getSlot()));
            e.setCancelled(callResult == null || callResult == CallResult.DENY_GRABBING);
        } catch (final Exception ex) {
            e.setCancelled(true);
