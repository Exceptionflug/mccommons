...
public class ItemBuilder {
    ...
    private static ArrayList<String> split(String string, int maxLength) {
        ...
        if (string.length() > 0) newString.add((newString.size() > 0 ? ChatColor.getLastColors(newString.get(newString.size() - 1)) : "") + string);
        return newString;
    }
    ...
    public ItemStack build() {
        ...
        return glow ? ItemUtils.addGlow(item) : item;
    }
